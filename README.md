...
- **Nushell** — primary shell. `config.nu` initializes the Starship prompt, sets `$env.EDITOR` to `nvim` when it is installed, hides the banner, aliases `ft` to `fastfetch` and `vim` to `nvim`, and sources `~/.cargo/env.nu`. `env.nu` only sets `$env.EDITOR`.
