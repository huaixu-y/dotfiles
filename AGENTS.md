...
- Be careful with absolute paths embedded in configs (`/usr/bin/nu`, `~/.cargo/env.nu`): they reflect the owner's machine and should stay parameterized by `$HOME`/`$nu.home-dir` where the config language allows.
