...
- **Nushell** — the primary shell. `config.nu` initializes the Starship prompt (regenerating `starship.nu` into `$nu.data-dir/vendor/autoload` on every startup), sets Helix (`hx`) as the buffer editor, hides the banner, aliases `ft` to `fastfetch` and `vim` to `nvim`, and sources `~/.cargo/env.nu`. `env.nu` prepends `~/.kimi-code/bin` to `PATH`. `nushell/history.txt` is machine-local and git-ignored via `nushell/.gitignore`.
